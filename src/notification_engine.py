...
Step 8 supports console, Linux desktop, Telegram, Discord webhook and SMTP
email notifications.  Backends are opt-in through environment variables and
fail independently so notification problems never stop packet capture or IDS
analysis.
"""
...
def notify_telegram(alert: dict[str, Any]) -> None:
    token = os.getenv("AI_IDS_TELEGRAM_BOT_TOKEN", "").strip()
    chat_id = os.getenv("AI_IDS_TELEGRAM_CHAT_ID", "").strip()
    if not token or not chat_id:
        raise RuntimeError(
            "AI_IDS_TELEGRAM_BOT_TOKEN and AI_IDS_TELEGRAM_CHAT_ID are required"
        )
...
def notify_discord(alert: dict[str, Any]) -> None:
    webhook_url = os.getenv("AI_IDS_DISCORD_WEBHOOK_URL", "").strip()
    if not webhook_url:
        raise RuntimeError("AI_IDS_DISCORD_WEBHOOK_URL is required")
...
def notify_email(alert: dict[str, Any]) -> None:
    host = os.getenv("AI_IDS_SMTP_HOST", "").strip()
    port = int(os.getenv("AI_IDS_SMTP_PORT", "587"))
    username = os.getenv("AI_IDS_SMTP_USERNAME", "").strip()
    password = os.getenv("AI_IDS_SMTP_PASSWORD", "")
    sender = os.getenv("AI_IDS_EMAIL_FROM", username).strip()
    recipients = [
        item.strip()
        for item in os.getenv("AI_IDS_EMAIL_TO", "").split(",")
        if item.strip()
    ]

    if not host or not sender or not recipients:
        raise RuntimeError(
            "AI_IDS_SMTP_HOST, AI_IDS_EMAIL_FROM and AI_IDS_EMAIL_TO are required"
        )
    ...
    use_ssl = _env_truthy("AI_IDS_SMTP_SSL", default=False)
    use_starttls = _env_truthy("AI_IDS_SMTP_STARTTLS", default=not use_ssl)
...
def notify(alert: dict[str, Any]) -> NotificationResult:
    """Dispatch an alert to every configured backend without failing the IDS."""
